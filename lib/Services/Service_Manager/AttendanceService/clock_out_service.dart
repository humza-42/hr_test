...
import 'package:hr_test/Models/attendance_mark_api_model.dart';
...
class AuthService {
  ...
  static Future<ClockOutAPI> clockOut() async {
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({'user_id': userId, 'action': 'clock_out'}),
      );

      if (response.statusCode == 200) {
        final Map<String, dynamic> responseData = jsonDecode(response.body);
        final clockOutResponse = ClockOutAPI.fromJson(responseData);

        if (clockOutResponse.ok == true) {
          return clockOutResponse;
        } else {
          throw Exception(
            clockOutResponse.message ?? 'Failed to clock out: Invalid response',
          );
        }
      } else {
