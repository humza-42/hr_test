...
class AuthService {
  ...
  static Future<BreakEndAPI> endBreak() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final breakEndResponse = BreakEndAPI.fromJson(responseData);

        if (breakEndResponse.ok == true) {
          return breakEndResponse;
        } else {
          throw Exception(
            breakEndResponse.message ?? 'Failed to end break: Invalid response',
          );
        }
      } else {
        ...
        final errorMessage =
            errorData?['error'] ??
            errorData?['message'] ??
            'Failed to end break: ${response.statusCode}';
